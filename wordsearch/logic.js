(function($, undefined) {
  ...
  var Visualizer = {
    ...
    signalWordFound: function(w) {
      ...
      $(w).css("background", 'yellow').animate({ "opacity": 'hide' }, 500, "linear",
        function() {
  ...
  var GameWidgetHelper = {
    ...
    renderGame: function(container, model) {
      ...
      var words = "<div id='rf-wordcontainer'><ol> word to find:";
      for (var i = 0; i < model.wordList.words.length; i++) {
        var word = model.wordList.words[i];
        if (i < 5) {
          words += "<li class='rf-p" + word.isPlaced + "'>" + word.originalValue + "</li>";
        } else if (i < 10) {
          words += "<li class='midle rf-p" + word.isPlaced + "'>" + word.originalValue + "</li>";
        } else {
          words += "<li class='right rf-p" + word.isPlaced + "'>" + word.originalValue + "</li>";
        }
      }
      words += `</ol>
      <section>
...
(function(b) {
  ...
  function d(g, h) {
    if (g.originalEvent.touches.length > 1) { return; } g.preventDefault();
  ...
  c._touchMove = function(f) {
    if (!a) { return; } this._touchMoved = true;
    d(f, "mousemove");
  };
  c._touchEnd = function(f) {
    if (!a) { return; } d(f, "mouseup");
    d(f, "mouseout");
    if (!this._touchMoved) { d(f, "click"); } a = false;
  };
...
$(document).ready(function() {
  var words = "Kado,Ucapan,Surprise,Suasana,Dekorasi,Manis,Bunga,Riang,Bahagia,Berbagi,Hangat,Seru,Lilin,Hadiah,Balon"
...
function setScore(){
  ...
  if (score == 15) {
    ...
    Cookies.set('acces3', 'true', { expires: 7 })
  }
}

window.onload = function() {
  let info = Cookies.get('info2')
  
  if (!info) {
   Swal.fire({
     title: 'Read me!',
     text: 'karna gw g bikin pure dari 0 gw gtau bug bug nya dmn jadi klo error refresh aja ya',
     icon: 'info',
     footer:'hope you enjoy it :)',
   }).then((result) => {
    if (result.isConfirmed) {
      Cookies.set('info2', 'true',{ expires: 7 })
    }
  })
  }
}